<?php
// ═══════════════════════════════════════════════════════════
//  classificacao.php — SOEE · Página de Esporte / Campeonato
//  Conectado ao banco real | URL: classificacao.php?id={id_modalidade}
// ═══════════════════════════════════════════════════════════
session_start();
...
// Só mostra modalidades que estão EM ANDAMENTO (não em inscrições)
$stmtEsportes = $conn->query("
    SELECT DISTINCT m.id_modalidade, m.nome_modalidade, m.tipo_modalidade,
                    m.formato_modalidade, em.status_edicao_modalidade
    FROM modalidade m
...
// ── DADOS COMPLETOS DA MODALIDADE SELECIONADA ────────────
$esporte = null;
$emId    = null;
$formato = null;
...
if ($emId && $temGrupos) {
    $stmtCl = $conn->prepare("
        ...
        ORDER BY cl.grupo_classificacao ASC,
                 cl.pontos DESC, cl.saldo DESC,
...
// ── VERIFICAR SE SORTEIO JÁ FOI GERADO ───────────────────
$sorteioGerado = false;
if ($emId) {
...
// Para individual/dupla/trio: "time" = a própria turma do aluno
...
function avatar($nome)   { return mb_strtoupper(mb_substr($nome ?? '?', 0, 2)); }
?>
...
<div class="app-layout">
                ...
                <a href="?id=<?= $esp['id_modalidade'] ?>" class="sidebar-item <?= $ativo ? 'ativo' : '' ?>">
                    ...
                    <div class="sidebar-item-info">
                        ...
                        <div class="sidebar-badges">
                            <span class="sidebar-badge andamento"><i class="fa-solid fa-circle" style="font-size:.35rem;vertical-align:middle"></i> Em Andamento</span>
                            <?php if ($fmtBadge): ?><span class="sidebar-badge formato"><?= $fmtBadge ?></span><?php endif; ?>
                        </div>
                    </div>
    ...
    <main class="conteudo" id="conteudo">
        ...
        <?php if (!$esporte): ?>
        <!-- NENHUMA MODALIDADE EM ANDAMENTO -->
            ...
            <div class="hero-inner">
                ...
                <div class="hero-texto">
                    ...
                    <div class="hero-meta">
                        ...
                        <span><i class="fa-solid fa-user"></i> <?= $participacaoLabel[$participacao] ?? ucfirst($participacao) ?></span>
                        <?php if (!empty($esporte['nome_edicao'])): ?>
                            <span><i class="fa-solid fa-trophy"></i> <?= htmlspecialchars($esporte['nome_edicao']) ?></span>
                        <?php endif; ?>
                        <span class="hero-status em_andamento"><i class="fa-solid fa-circle" style="font-size:.45rem;vertical-align:middle"></i> Em Andamento</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="tab-conteudo ativo" id="tab-grupos">
            <?php if (empty($grupos)): ?>
                <div class="empty-state">
                    <i class="fa-solid fa-table-cells"></i>
                    <p>O sorteio ainda não foi realizado ou nenhuma partida foi jogada.</p>
                </div>
            <?php else: ?>

            <?php /* Aviso de participação individual */ ?>
            <?php if ($ehIndividual): ?>
            ...
            <div class="grupos-grid">
                ...
                <div class="grupo-card reveal">
                    <div class="grupo-header">
                        <div class="grupo-letra"><?= $formato === 'todos_contra_todos' ? '#' : $nomeGrupo ?></div>
                        <div>
                            <div class="grupo-titulo">
                                <?= $formato === 'todos_contra_todos' ? 'Classificação Geral' : 'Grupo ' . $nomeGrupo ?>
                            </div>
                            <div class="grupo-sub"><?= count($times) ?> time<?= count($times) > 1 ? 's' : '' ?></div>
                        </div>
                        <?php if ($formato !== 'todos_contra_todos'): ?>
                        <div class="grupo-qualifica">
                            <i class="fa-solid fa-arrow-up"></i> Top <?= $classificamPorGrupo ?> avançam
                        </div>
                        <?php endif; ?>
                    </div>
                        ...
                        <table class="grupo-tabela">
                            <thead>
                                <tr>
                                    ...
                                    <th title="Pontos a favor (gols/sets marcados)">GP</th>
                                    <th title="Pontos contra (gols/sets sofridos)">GC</th>
                                    <th title="Saldo (GP - GC)">SG</th>
                                    <th title="Pontos na tabela">PTS</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($times as $pos => $time):
                                    $classifica = $pos < $classificamPorGrupo && $formato !== 'grupos' && $formato !== 'todos_contra_todos';
                                    $liderde    = $pos === 0;
                                    $saldo      = (int) $time['saldo'];
                                ?>
                                <tr class="<?= $classifica ? 'classificado' : '' ?> <?= $liderde ? 'lider' : '' ?>">
                                    <td class="td-pos">
                                        <?php if ($pos === 0): ?>
                                            <span class="pos-badge ouro" title="1º lugar do grupo">1</span>
                                        <?php elseif ($pos === 1): ?>
                                            <span class="pos-badge prata" title="2º lugar do grupo">2</span>
                                        <?php elseif ($pos === 2): ?>
                                    ...
                                    <td class="td-time">
                                        <div class="time-col">
                                            <div class="time-avatar"><?= avatar($time['nome_turma']) ?></div>
                                            <span class="time-nome"><?= htmlspecialchars($time['nome_turma']) ?></span>
                                            <?php if ($classifica): ?>
                                                <span class="badge-classifica" title="Classificado para próxima fase"><i class="fa-solid fa-check"></i></span>
                                            <?php endif; ?>
                                        </div>
                                    </td>
                    ...
                    <div class="grupo-jogos">
                        <div class="grupo-jogos-titulo"><i class="fa-solid fa-futbol"></i> Jogos do Grupo <?= $nomeGrupo ?></div>
                ...
                <div class="bracket">
                    ...
                    <div class="bracket-coluna <?= $isFinal ? 'coluna-final' : '' ?> <?= $is3o ? 'coluna-3o' : '' ?>">
                        ...
                        <div class="bracket-jogos">
                            ...
                            ?>
                            <div class="bracket-jogo <?= $isFinal ? 'jogo-final' : '' ?> <?= $r ? 'realizada' : '' ?> reveal">
                                <div class="bracket-time <?= !empty($jogo['vencedor']) && $jogo['vencedor'] === $jogo['time_a'] ? 'vencedor' : '' ?>">
                                    <div class="bracket-avatar"><?= avatar($jogo['time_a'] ?? '?') ?></div>
                                    <span><?= htmlspecialchars($jogo['time_a'] ?: 'A definir') ?></span>
                                    <?php if ($r): ?><strong class="bracket-placar"><?= $jogo['placar_time_a'] ?></strong><?php endif; ?>
                                    <?php if (!empty($jogo['vencedor']) && $jogo['vencedor'] === $jogo['time_a']): ?><i class="fa-solid fa-check bracket-check"></i><?php endif; ?>
                                </div>
                                <div class="bracket-sep">
                                    <?php if ($wo): ?>
                                        <span class="bracket-wo">W.O.</span>
                                    <?php elseif (!$r): ?>
                                        <span class="bracket-data"><?= fmtData($jogo['data_partida']) ?> <?= fmtHora($jogo['hora_partida']) ?></span>
                                    <?php else: ?>
                                        <span>×</span>
                                    <?php endif; ?>
                                </div>
                                <div class="bracket-time <?= !empty($jogo['vencedor']) && $jogo['vencedor'] === $jogo['time_b'] ? 'vencedor' : '' ?>">
                                    <div class="bracket-avatar"><?= avatar($jogo['time_b'] ?? '?') ?></div>
                                    <span><?= htmlspecialchars($jogo['time_b'] ?: 'A definir') ?></span>
                                    <?php if ($r): ?><strong class="bracket-placar"><?= $jogo['placar_time_b'] ?></strong><?php endif; ?>
                                    <?php if (!empty($jogo['vencedor']) && $jogo['vencedor'] === $jogo['time_b']): ?><i class="fa-solid fa-check bracket-check"></i><?php endif; ?>
                                </div>
                                <?php if (!empty($jogo['local_partida'])): ?>
                                <div class="bracket-local"><i class="fa-solid fa-location-dot"></i> <?= htmlspecialchars($jogo['local_partida']) ?></div>
                                <?php endif; ?>
                            </div>
                            <?php endforeach; ?>
                        </div>
            ...
            <div class="partidas-lista">
                ...
                <div class="partidas-secao reveal">
                    ...
                    <?php foreach ($partidas_fase[$fase] as $p):
                        $r  = $p['status_partida'] === 'realizada';
                        $wo = $p['status_partida'] === 'wo';
                    ?>
                    <div class="partida-row <?= $r ? 'realizada' : '' ?> <?= $fase === 'final' ? 'partida-final' : '' ?>">
                        <div class="pr-times">
                            <span class="pr-time <?= !empty($p['vencedor']) && $p['vencedor'] === $p['time_a'] ? 'vencedor' : '' ?>">
                            ...
                            <span class="pr-time direita <?= !empty($p['vencedor']) && $p['vencedor'] === $p['time_b'] ? 'vencedor' : '' ?>">
        ...
        <?php endif; // fim se esporte existe ?>
    </main>
</div>
